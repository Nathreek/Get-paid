...
export function createAppServer({directory,store,rateLimit=5,trustProxy=false,background=promise=>promise}){
  ...
  async function body(req){
    ...
    for await(const part of req){size+=part.length;if(size>16384)throw Object.assign(new Error('Request is too large.'),{status:413});parts.push(part);}
    ...
    try{
      ...
      if(url.pathname.startsWith('/api/')){
        ...
        if(req.method==='GET'&&url.pathname==='/api/state'){
          ...
          payouts();
          const state=await store.state(raw);const etag=`"${state.revision}-${state.page}"`;
        ...
        if(req.method==='POST'&&url.pathname==='/api/submissions'){
          ...
          const record=await store.submit(input.post,input.wallet);payouts();json(res,201,{record});return;
        }
