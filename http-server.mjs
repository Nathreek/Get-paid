import http from 'node:http';
import path from 'node:path';
import { readFile, stat } from 'node:fs/promises';
import { randomBytes, createHash } from 'node:crypto';
const mime={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.png':'image/png','.svg':'image/svg+xml'};
export function createAppServer({directory,store,secureCookies=false,rateLimit=60}){
  const root=path.resolve(directory),cache=new Map(),limits=new Map();
  function json(res,status,body){res.writeHead(status,{'Content-Type':'application/json','Cache-Control':'no-store'});res.end(JSON.stringify(body));}
  function owner(req,res){
    let token=/\bgp_session=([a-f0-9]{64})(?:;|$)/.exec(req.headers.cookie||'')?.[1];
    if(!token){token=randomBytes(32).toString('hex');res.setHeader('Set-Cookie',`gp_session=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=31536000${secureCookies?'; Secure':''}`);}
    return createHash('sha256').update(token).digest('hex');
  }
  function allowed(key,maximum=rateLimit){
    const now=Date.now();let value=limits.get(key);
    if(!value||value.until<=now){if(limits.size>10000){for(const [id,item] of limits)if(item.until<=now)limits.delete(id);if(limits.size>10000)limits.delete(limits.keys().next().value);}value={count:0,until:now+60000};limits.set(key,value);}
    return ++value.count<=maximum;
  }
  async function body(req){
    if(!(req.headers['content-type']||'').startsWith('application/json'))throw Object.assign(new Error('Send JSON data.'),{status:415});
    let size=0;const parts=[];
    for await(const part of req){size+=part.length;if(size>(req.url==='/api/import'?262144:16384))throw Object.assign(new Error('Request is too large.'),{status:413});parts.push(part);}
    try{return JSON.parse(Buffer.concat(parts).toString());}catch{throw Object.assign(new Error('Invalid JSON request.'),{status:400});}
  }
  async function asset(file){
    const old=cache.get(file);if(old&&Date.now()-old.checkedAt<1000)return old.promise;
    const item={checkedAt:Date.now()};item.promise=(async()=>{const info=await stat(file);if(!info.isFile()||info.size>10*1024*1024)throw new Error('Not found');const previous=old&&await old.promise.catch(()=>null);if(previous?.mtime===info.mtimeMs)return previous;const data=await readFile(file);return{data,mtime:info.mtimeMs,etag:`"${createHash('sha256').update(data).digest('hex')}"`};})();
    if(cache.size>=128&&!old)cache.delete(cache.keys().next().value);cache.set(file,item);
    try{return await item.promise;}catch(error){cache.delete(file);throw error;}
  }
  const server=http.createServer(async(req,res)=>{
    res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','strict-origin-when-cross-origin');
    try{
      const url=new URL(req.url,'http://localhost');
      if(url.pathname==='/healthz'){json(res,200,{status:'ok'});return;}
      if(url.pathname.startsWith('/api/')){
        const visitor=owner(req,res);
        if(!['GET','HEAD'].includes(req.method)){
          const origin=req.headers.origin;let originHost;try{originHost=new URL(origin).host;}catch{}
          if(!originHost||originHost!==req.headers.host){json(res,403,{error:'This request must come from this website.'});return;}
          if(url.pathname==='/api/submissions'&&!allowed(visitor)){res.setHeader('Retry-After','60');json(res,429,{error:'Too many submissions. Please wait a minute and try again.'});return;}
        }
        if(req.method==='GET'&&url.pathname==='/api/state'){
          const raw=Number(url.searchParams.get('page')||0);if(!Number.isInteger(raw)||raw<0){json(res,400,{error:'Invalid page.'});return;}
          const state=store.state(visitor,raw);const etag=`"${visitor.slice(0,16)}-${state.revision}-${state.page}-${state.spotlight.changedAt}"`;
          res.setHeader('ETag',etag);res.setHeader('Cache-Control','no-cache, private');
          if(req.headers['if-none-match']===etag){res.writeHead(304).end();return;}
          json(res,200,state);return;
        }
        if(req.method==='POST'&&url.pathname==='/api/submissions'){
          const input=await body(req);if(typeof input.post!=='string'||typeof input.handle!=='string'){json(res,400,{error:'Provide a post URL and X handle.'});return;}
          const record=await store.add(visitor,input.post,input.handle);json(res,201,{record});return;
        }
        if(req.method==='POST'&&url.pathname==='/api/import'){
          if(!allowed(visitor+':import',5)){json(res,429,{error:'Please wait a minute before importing more saved entries.'});return;}
          const input=await body(req);
          if(!Array.isArray(input.entries)||input.entries.length>100){json(res,400,{error:'Import up to 100 entries at a time.'});return;}
          const results=[];
          for(const entry of input.entries){
            try{const record=await store.add(visitor,entry.post,entry.handle);results.push({legacyId:entry.id,imported:true,id:record.id});}
            catch(error){results.push({legacyId:entry.id,imported:error.status===409,error:error.message});}
          }
          json(res,200,{results});return;
        }
        if(req.method==='POST'&&url.pathname==='/api/spotlight/seen'){
          const input=await body(req);if(typeof input.id!=='string'||input.id.length>80){json(res,400,{error:'Invalid submission.'});return;}
          json(res,200,{recorded:store.acknowledge(input.id)});return;
        }
        if(req.method==='DELETE'&&/^\/api\/submissions\/[a-f0-9-]+$/.test(url.pathname)){
          const id=url.pathname.split('/').at(-1);const removed=store.remove(visitor,id);json(res,removed?200:404,removed?{removed:true}:{error:'This entry is unavailable or belongs to another visitor.'});return;
        }
        json(res,404,{error:'Not found'});return;
      }
      if(!['GET','HEAD'].includes(req.method)){res.writeHead(405,{Allow:'GET, HEAD'}).end();return;}
      const name=decodeURIComponent(url.pathname==='/'?'/index.html':url.pathname);
      if(name.includes('\\')||name.includes('\0')||name.split('/').some(part=>part.startsWith('.'))){res.writeHead(404).end();return;}
      const file=path.resolve(root,'.'+name),type=mime[path.extname(file)];
      if(!file.startsWith(root+path.sep)||!type){res.writeHead(404).end();return;}
      const result=await asset(file);res.setHeader('Content-Type',type);res.setHeader('Cache-Control','no-cache');res.setHeader('ETag',result.etag);
      if(req.headers['if-none-match']===result.etag){res.writeHead(304).end();return;}
      res.writeHead(200,{'Content-Length':result.data.length});res.end(req.method==='HEAD'?undefined:result.data);
    }catch(error){if(res.headersSent){res.end();return;}const status=error.status||(error.code==='ENOENT'?404:400);json(res,status,{error:status===404?'Not found':error.message||'Request failed. Please try again.'});}
  });
  server.requestTimeout=20000;server.headersTimeout=10000;server.keepAliveTimeout=5000;
  return server;
}
